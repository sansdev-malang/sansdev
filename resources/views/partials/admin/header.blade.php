    ...
    <div class="flex items-center gap-2">
        ...
        <div x-data="{ open: false }" class="relative">
            <button @click="open = !open"
                class="flex items-center rounded-full border border-slate-200 dark:border-slate-800 overflow-hidden focus:outline-none focus:ring-2 focus:ring-slate-400 dark:focus:ring-slate-650">
            ...
            <div x-show="open" @click.outside="open = false"
                class="absolute right-0 mt-2 w-48 bg-white dark:bg-[#09090b] border border-slate-200 dark:border-slate-800 rounded-md shadow-lg py-1"
                x-transition>
                <a href="{{ route('profile.edit') }}"
                class="block px-3 py-2 text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-900/50 hover:text-slate-900 dark:hover:text-slate-100 transition-colors">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                        class="w-full text-left px-3 py-2 text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-900/50 hover:text-slate-900 dark:hover:text-slate-100 transition-colors">
